...
def stop_recording():
    ...
    logging.info(f"Request JSON data: {request.get_json()}")
    ...
    logging.info(f"input_langugae receive :{input_language}")
    logging.info(f"output language receive :{output_language}")
    ...
    logging.info(f"Input text is :{input_text}")
    ...
    logging.info(f"Translated text is :",translated_text)
    # Text-to-Speech (TTS)
    ...
    # Send the new audio URL back in the response with the full path (localhost:5000)
